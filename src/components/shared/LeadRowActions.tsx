...
import { Eye, ArrowRightLeft, Phone, PhoneForwarded, MessageCircle } from "lucide-react";
        ...
        {ACTIONS.map(({ key, label, Icon, onClickKey, className }) => (
          <button
            ...
            className={cn(
              ...
              className,
              key === "ivr" && ivrLoading && "opacity-60",
            )}
          >
            <Icon className="h-4 w-4 shrink-0" aria-hidden />
            {key === "wa" ? "WhatsApp" : key === "ivr" ? (ivrLoading ? "IVR…" : "IVR") : label.replace(" lead", "").replace(" stage", "")}
          </button>
        ))}
      </div>
    );
